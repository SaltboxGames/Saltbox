...
{
    public static class AsyncQueueExtentions
    {
        private static IEnumerator WaitDequeue<T>(this AsyncQueue<T> queue, Action<T> onDequeue)
        {
            Task<T> waiter = queue.DequeueAsync();
            while(!waiter.IsCompleted)
            {
                yield return null;
            }
            onDequeue?.Invoke(waiter.Result);
        }
    }
